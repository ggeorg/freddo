...
@implementation FdContact : NSObject
...
+ (NSDictionary*)defaultFields
{
    if (org_apache_cordova_contacts_defaultFields == nil) {
        org_apache_cordova_contacts_defaultFields = [NSDictionary dictionaryWithObjectsAndKeys:
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactName], kW3ContactName,
            [NSNull null], kW3ContactNickname,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactAddresses], kW3ContactAddresses,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactOrganizations], kW3ContactOrganizations,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactPhoneNumbers], kW3ContactPhoneNumbers,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactEmails], kW3ContactEmails,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactIms], kW3ContactIms,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactPhotos], kW3ContactPhotos,
            [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactUrls], kW3ContactUrls,
...
- (bool)setFromContactDict:(NSDictionary*)aContact asUpdate:(BOOL)bUpdate
{
    ...
    if ([dict isKindOfClass:[NSDictionary class]]) {
        bName = true;
        NSArray* propArray = [[FdContact defaultObjectAndProperties] objectForKey:kW3ContactName];
        ...
        for (id i in propArray) {
            if (![(NSString*)i isEqualToString : kW3ContactFormattedName]) { // kW3ContactFormattedName is generated from ABRecordCopyCompositeName() and can't be set
                [self setValue:[dict valueForKey:i] forProperty:(ABPropertyID)[(NSNumber*)[[FdContact defaultW3CtoAB] objectForKey:i] intValue]
...
- (bool)addToMultiValue:(ABMultiValueRef)multi fromDictionary:dict
{
    ...
    if (IS_VALID_VALUE(value)) {
        CFStringRef label = [FdContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
    ...
    if (!bUpdate) {
        ...
        if (copy != nil) {
            ...
            for (NSDictionary* dict in fieldArray) {
                ...
                label = (__bridge NSString*)[FdContact convertContactTypeToPropertyLabel:[dict valueForKey:kW3ContactFieldType]];
...
{
    NSArray* propArray = [[FdContact defaultObjectAndProperties] valueForKey:[[FdContact defaultABtoW3C] objectForKey:[NSNumber numberWithInt:prop]]];
    ...
    for (NSString* key in propArray) { // for each W3 Contact key get the value
        if (((value = [dict valueForKey:key]) != nil) && ![value isKindOfClass:[NSNull class]]) {
            ...
            if ([FdContact needsConversion:key]) { // IM types must be converted
                setValue = (NSString*)[FdContact convertContactTypeToPropertyLabel:value];
            ...
            [newDict setObject:setValue forKey:(NSString*)[[FdContact defaultW3CtoAB] valueForKey:(NSString*)key]];
        }
    }
            ...
            for (NSDictionary* field in array) {
                ...
                if (idx != kCFNotFound) {
                    ...
                    if (testLabel && [testLabel isKindOfClass:[NSString class]] && ([testLabel length] > 0)) {
                        CFStringRef w3cLabel = [FdContact convertContactTypeToPropertyLabel:testLabel];
                    ...
                    for (id k in propArray) {
                        ...
                        if (bSet) {
                            NSString* setValue = [FdContact needsConversion:(NSString*)k] ? (NSString*)[FdContact convertContactTypeToPropertyLabel:value] : value;
                            [dict setObject:setValue forKey:(NSString*)[[FdContact defaultW3CtoAB] valueForKey:(NSString*)k]];
                            ...
                            valueAB = [(__bridge NSDictionary*)existingDictionary valueForKey : [[FdContact defaultW3CtoAB] valueForKey:k]];
                            if (valueAB != nil) {
                                [dict setValue:valueAB forKey:[[FdContact defaultW3CtoAB] valueForKey:k]];
                            }
...
- (NSObject*)extractName
{
    ...
    for (NSString* i in fields) {
        if ([i isEqualToString:kW3ContactFormattedName]) {
            ...
            value = (__bridge_transfer NSString*)ABRecordCopyValue(self.record, (ABPropertyID)[[[FdContact defaultW3CtoAB] valueForKey:i] intValue]);
...
- (NSObject*)extractMultiValue:(NSString*)propertyId
{
    ...
    NSNumber* propNumber = [[FdContact defaultW3CtoAB] valueForKey:propertyId];
    ...
    if (count) {
        ...
        for (CFIndex i = 0; i < count; i++) {
            ...
            if ([fields containsObject:kW3ContactFieldType]) {
                ...
                value = [FdContact convertPropertyLabelToContactType:label];
...
- (NSObject*)extractAddresses
{
    ...
    if (count) {
        ...
        for (CFIndex i = 0; i < count; i++) {
            ...
            [newAddress setObject:(label != nil) ? (NSObject*)[[FdContact class] convertPropertyLabelToContactType:label]:[NSNull null] forKey:kW3ContactFieldType];
            ...
            for (id k in fields) {
                bool bFound;
                id key = [[FdContact defaultW3CtoAB] valueForKey:k];
    ...
    if (count) {
        ...
        for (CFIndex i = 0; i < ABMultiValueGetCount(multi); i++) {
            ...
            if ([fields containsObject:kW3ContactFieldType]) {
                ...
                if (bFound && (value != NULL)) {
                    CFRetain(value);
                    [newDict setObject:(id)[[FdContact class] convertPropertyLabelToContactType : (__bridge NSString*)value] forKey:kW3ContactFieldType];
                    CFRelease(value);
                } else {
...
- (NSObject*)extractOrganizations
{
    ...
    for (id i in fields) {
        id key = [[FdContact defaultW3CtoAB] valueForKey:i];
...
+ (NSDictionary*)calcReturnFields:(NSArray*)fieldsArray // NSLog(@"getting self.returnFields");
{
    ...
    if ((fieldsArray != nil) && [fieldsArray isKindOfClass:[NSArray class]]) {
        if (([fieldsArray count] == 1) && [[fieldsArray objectAtIndex:0] isEqualToString:@"*"]) {
            return [FdContact defaultFields];  // return all fields
        }
        ...
        for (id i in fieldsArray) {
            ...
            id fields = [[FdContact defaultObjectAndProperties] objectForKey:name];
            ...
            if ((fields != nil) && (property == nil)) { // request was for full object
                ...
                id abEquiv = [[FdContact defaultW3CtoAB] objectForKey:property];
                if (abEquiv || [[FdContact defaultW3CtoNull] containsObject:property]) {
                ...
                id valid = [[FdContact defaultW3CtoAB] objectForKey:name];
                if (valid || [[FdContact defaultW3CtoNull] containsObject:name]) {
...
- (BOOL)testStringValue:(NSString*)testValue forW3CProperty:(NSString*)property
{
    ...
    if ([[FdContact defaultW3CtoAB] valueForKeyIsNumber:property]) {
        ABPropertyID propId = [[[FdContact defaultW3CtoAB] objectForKey:property] intValue];
...
- (BOOL)searchContactFields:(NSArray*)fields forMVStringProperty:(ABPropertyID)propId withValue:testValue
{
    ...
    for (NSString* type in fields) {
        ...
        if ([type isEqualToString:kW3ContactFieldType]) {
            if ([FdContact isValidW3ContactType:testValue]) {
                // only search types if the filter string is a valid ContactField.type
                testString = (NSString*)[FdContact convertContactTypeToPropertyLabel:testValue];
            }
        } else {
...
- (BOOL)searchContactFields:(NSArray*)fields forMVDictionaryProperty:(ABPropertyID)propId withValue:(NSString*)testValue
{
    ...
    for (int i = 0; i < dictCount; i++) {
        ...
        for (NSString* member in fields) {
            NSString* abKey = [[FdContact defaultW3CtoAB] valueForKey:member]; // im and address fields are all strings
